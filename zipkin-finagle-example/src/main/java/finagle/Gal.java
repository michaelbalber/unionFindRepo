...
public class Gal extends Service<Request, Response> {

  final Service<Request, Response> backendClient;

  Gal(Service<Request, Response> backendClient) {
  ...
  public Future<Response> apply(Request request) {
    if (request.uri().equals("/")) {
      return backendClient.apply(Request.apply("/"));
    }
  ...
  public static void main(String[] args) {
    ...
    new Flags("gal", true, true).parseOrExit1(args, false);
    ...
    Service<Request, Response> backendClient =
        ClientBuilder.safeBuild(ClientBuilder.get()
            ...
            .name("gal")); // this assigns the local service name

    ServerBuilder.safeBuild(
        new Gal(backendClient),
        ServerBuilder.get()
            .tracer(tracer)
            .codec(Http.get().enableTracing(true))
            .bindTo(new InetSocketAddress(InetAddress.getLoopbackAddress(), 8080))
            .name("gal")); // this assigns the local service name
  }
}
